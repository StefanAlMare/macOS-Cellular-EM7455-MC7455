...
int main(void)
{
    ...
    printf("CELLULAR IPv6 CAPABILITY PROBE v1.0\n");
    printf("Temporary dual-stack bearer; no macOS routes or DNS changes.\n");
    printf("CONNECT IP type = IPv4v6 (MBIM value 3).\n\n");
    ...
    /* Keep the same proven preflight sequence. */
    ...
    printf("\n==================================================\n");
    printf("CONNECT broadband / IPv4v6\n");
    printf("==================================================\n");
    ...
    if (!bearer_activated) {
        printf("\nDUAL-STACK CONNECT FAILED.\n");
        goto close_mbim;
    }

    printf("\n*** DUAL-STACK CONNECT ACCEPTED ***\n");

    int saw_v6_config = 0;
    int saw_v6_packets = 0;

    saw_v6_config |= query_ip_configuration_probe(
        h, 730, resp, sizeof(resp), 1
    );

    saw_v6_packets |= sniff_ipv6_packets(h, 2500);

    sleep(1);

    saw_v6_config |= query_ip_configuration_probe(
        h, 731, resp, sizeof(resp), 2
    );

    saw_v6_packets |= sniff_ipv6_packets(h, 3000);

    sleep(1);

    saw_v6_config |= query_ip_configuration_probe(
        h, 732, resp, sizeof(resp), 3
    );

    printf("\n==================================================\n");
    printf("PROBE SUMMARY\n");
    printf("==================================================\n");
    printf("Dual-stack CONNECT accepted : YES\n");
    printf("IPv6 in IP_CONFIGURATION    : %s\n",
           saw_v6_config ? "YES" : "NO");
    printf("IPv6 packets on MBIM data   : %s\n",
           saw_v6_packets ? "YES" : "NO");

    if (saw_v6_config || saw_v6_packets) {
        printf("RESULT: IPv6 bearer evidence detected.\n");
    } else {
        printf(
            "RESULT: no IPv6 evidence during this probe; "
            "network may be IPv4-only for this bearer/APN "
            "or may require host-side RA/SLAAC solicitation.\n"
        );
    }

close_mbim:
    if (bearer_activated) {
        printf("\nDeactivating temporary Session 0...\n");
        ...
        printf("Temporary bearer deactivation requested.\n");
    }
