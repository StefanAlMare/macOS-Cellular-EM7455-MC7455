...
static uint32_t g_probe_ip_type = 3;
    ...
    int max_wait_seconds)
{
    ...
    put32(ci + 40, g_probe_ip_type);     /* dynamic probe IP type */
    ...
    printf(
        "Requested IP type: %u (%s)\n",
        g_probe_ip_type,
        g_probe_ip_type == 2 ? "IPv6-only" :
        g_probe_ip_type == 3 ? "IPv4v6" :
        g_probe_ip_type == 1 ? "IPv4" : "other"
    );
    ...
    size_t resp_size)
{
    ...
    put32(di + 32, 0);
    put32(di + 36, 0);
    put32(di + 40, g_probe_ip_type);      /* dynamic probe IP type */
...
static int query_ip_configuration_probe(
    libusb_device_handle *h,
    uint32_t tid,
    unsigned char *resp,
    size_t resp_size,
    int ordinal)
{
    ...
    put32(ipq + 44, 60);       /* fixed query structure */

    printf("\n==================================================\n");
    printf("IP_CONFIGURATION QUERY #%d\n", ordinal);
    printf("==================================================\n");
    ...
    if ((v4flags & 0x1) &&
        ...
        unsigned char *v4e = ip + v4off;
        uint32_t prefix4 = le32(v4e);

        printf("IPv4       : %u.%u.%u.%u/%u\n",
               v4e[4], v4e[5], v4e[6], v4e[7], prefix4);
    } else {
    ...
    if ((v6flags & 0x1) &&
        v6count > 0 &&
        v6off != 0 &&
        v6off + (v6count * 20) <= infolen) {

        for (uint32_t i = 0; i < v6count; i++) {
            unsigned char *v6e = ip + v6off + i * 20;
            uint32_t prefix6 = le32(v6e);
        ...
        printf("IPv6 addr  : not supplied via MBIM IP_CONFIGURATION\n");
    }
    ...
    if ((v6flags & 0x2) &&
        ...
        printf("IPv6 GW    : not supplied via MBIM IP_CONFIGURATION\n");
    }

    if ((v6flags & 0x4) &&
        dns6count > 0 &&
        dns6off != 0 &&
        dns6off + dns6count * 16 <= infolen) {

        for (uint32_t i = 0; i < dns6count; i++) {
            char text[INET6_ADDRSTRLEN];
            memset(text, 0, sizeof(text));
        ...
        printf("IPv6 DNS   : not supplied via MBIM IP_CONFIGURATION\n");
    }
...
static int sniff_ipv6_packets(
    libusb_device_handle *h,
    int milliseconds)
{
    /*
     * Configure NTB16 and input size exactly like the stable bridge,
     * then only READ the MBIM/NCM data endpoint.
     */
        ...
        while (ndpo &&
            ...
            if ((sig & 0x00ffffff) == 0x00535049 &&
                ...
                for (int pos = ndpo + 8;
                    ...
                    printf(
                        "IPv6 packet #%d: nh=%u src=%s dst=%s len=%d\n",
                        v6_packets,
                        next_header,
                        src,
                        dst,
                    ...
                    if (next_header == 58 && len >= 41) {
                        uint8_t icmp_type = pkt[40];

                        printf("  ICMPv6 type=%u\n", icmp_type);

                        if (icmp_type == 134) {
                            ra_packets++;
                            printf("  *** ROUTER ADVERTISEMENT DETECTED ***\n");
                        }
                    }
                }
            }
    ...
    printf(
        "Sniff result: IPv6 packets=%d, Router Advertisements=%d\n",
        v6_packets,
        ra_packets
    );
    ...
    uint32_t base_tid)
{
    ...
    g_probe_ip_type = result->requested_ip_type;

    printf("\n\n");
    printf("############################################################\n");
    printf("SCENARIO: %s\n", result->name);
    printf("APN     : %s\n",
           result->apn[0] ? result->apn : "(NULL / network default)");
    printf("IP type : %u (%s)\n",
           result->requested_ip_type,
           result->requested_ip_type == 2 ? "IPv6-only" :
           result->requested_ip_type == 3 ? "IPv4v6" : "other");
    printf("############################################################\n");

    result->activated = mbim_activate_session0_apn(
        h,
        result->apn,
        base_tid,
        resp,
    ...
    if (!result->activated) {
        printf("\nRESULT: CONNECT FAILED / bearer not activated.\n");
        result->ipv6_config = 0;
        result->ipv6_packets = 0;

        /* Clean Session 0 even after a failed attempt. */
    ...
    result->ipv6_config = query_ip_configuration_probe(
        h,
        base_tid + 20,
        resp,
        resp_size,
    ...
    result->ipv6_packets = sniff_ipv6_packets(h, 1800);
    ...
    printf("Scenario deactivation requested.\n");
    sleep(1);
}
...
int main(void)
{
    ...
    printf("CELLULAR IPv6 MATRIX PROBE v1.0\n");
    printf("Temporary MBIM tests only; installed Cellular.app is untouched.\n");
    printf("No macOS routes, DNS, utun, Wi-Fi or Ethernet changes.\n\n");
    ...
    /* Exact stable initialization sequence. */
    ...
    if (r < 16 || le32(resp + 12) != 0) {
        printf("MBIM_OPEN failed\n");
        goto cleanup;
    }

    printf("*** MBIM OPEN OK ***\n");

    /* Same proven preflight sequence as stable engine. */
    (void)send_query(h, 2, 501, resp, sizeof(resp));
    (void)send_query(h, 9, 502, resp, sizeof(resp));
    (void)send_query(h, 10, 503, resp, sizeof(resp));
    (void)send_query(h, 11, 504, resp, sizeof(resp));

    struct scenario_result scenarios[] = {
        {
            "A - broadband + IPv4v6",
            "broadband",
            3,
    ...
    for (size_t i = 0; i < count; i++) {
        printf(
            ...
            "  IPv6 packets      : %s\n",
            scenarios[i].name,
            scenarios[i].activated ? "YES" : "NO",
            scenarios[i].ipv6_config ? "YES" : "NO",
            scenarios[i].ipv6_packets ? "YES" : "NO"
        );
    }

    printf("\nInterpretation:\n");
    printf(
        "- If either IPv6-only scenario activates, native IPv6 exists.\n"
    );
    printf(
        "- If IPv4v6 activates only as IPv4 and both IPv6-only "
        "scenarios fail, the current mobile bearer is IPv4-only.\n"
    );
    printf(
        "- If NULL APN differs from broadband, we can select the "
        "dual-stack-capable context in Cellular.app.\n"
    );
